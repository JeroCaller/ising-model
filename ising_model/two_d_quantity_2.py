'''
ver2
본격적인 메트로폴리스 알고리즘 실행에 앞서 먼저 물리량 변동을 줄이고자 격자 상태가 평형상태에 들어갈 때까지
미리 메트로폴리스 알고리즘을 선실행한다. 
'''
...
class IsingModel():
    def __init__(self):
        ...
        for t in self.T:
            ...
            #    self.metropolis_algorithm(t)  # 평형상태에 도달할때까지 선 실행.
            #self.init_latt = self.lattice.copy() 
            for er in range(self.err):
                self.lattice = self.init_latt.copy()
                energy = 0
                magnet = 0
                for tr in range(self.transition):
                    self.metropolis_algorithm(t)
                    energy += self.calc_current_energy()
                    magnet += self.calc_current_mag()
                energies.append((-(self.J / self.transition) * energy) / (self.L**2))
                magnets.append((magnet / self.transition) / (self.L**2))
    ...
    def save_data(self, file_name, field):
        ...
        with open(file_name, 'a') as f:
            f.write('{},{},{},{},{},{},{}\n'.format(size, transition, run, error, T, energy_m, magnet_m))
...
ising.save_data('two_d_quantity.txt', 'size,transition,run,error,T,energy_m,magnet_m')
